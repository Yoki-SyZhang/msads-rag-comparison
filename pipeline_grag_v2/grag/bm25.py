"""Small serializable BM25 index with domain-common lexical terms removed."""
...
class BM25Index:
    def __init__(self, texts: Iterable[str], k1: float = 1.5, b: float = 0.75) -> None:
        self.k1 = k1
        self.b = b
        self.docs = [tokenize(text, remove_domain_terms=True) for text in texts]
        ...
        self.term_freqs: List[Counter[str]] = [Counter(doc) for doc in self.docs]
    ...
    def scores(self, query: str) -> np.ndarray:
        terms = tokenize(query, remove_domain_terms=True)
        ...
        for term in terms:
            ...
            for index, freqs in enumerate(self.term_freqs):
                ...
                if not tf:
                    continue
                denom = tf + self.k1 * (
                    1 - self.b + self.b * self.doc_lens[index] / max(self.avgdl, 1e-6)
                )
